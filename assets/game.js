(function(){
    var tetris = {
        ...
        bindTouchEvents: function() {
          ...
          leftButton.addEventListener('touchstart', () => this.move('L'));
          rightButton.addEventListener('touchstart', () => this.move('R'));
          upButton.addEventListener('touchstart', () => this.move('RT'));
          downButton.addEventListener('touchstart', () => this.move('D'));
      },
        handleKey: function(e){
            ...
            switch (c){
                ...
                case 27: //esc: pause
                    this.togglePause();
                    break;
                case 32: // Spacebar: Drop block to the bottom
                    this.dropToBottom();
        ...
        dropToBottom: function() {
            var tempY = this.curY;
        
            // Increment Y position until the block can't move further down
            while (this.checkMove(this.curX, tempY + 1, this.curShape)) {
                tempY++;
            }
        
            // Remove the block from the current position and redraw it at the lowest valid position
            this.removeCur();
            this.curY = tempY;
            this.drawShape(this.curX, this.curY, this.curShape);
        ...
        checkRows: function() {
            ...
            if (clearedRows > 0) {
                this.calcScore({ lines: clearedRows });
            }
        },
